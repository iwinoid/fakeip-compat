...
本插件注册了 `fakeip-compat` 设置节 **和** 同名浏览器卡片（`lib/client.js`，
手写、无构建步骤）：重启 DSH 后在插件配置页会出现「Fake-IP 兼容抓取」卡片，
上表所有字段都可以在 Web GUI 里可视化编辑，保存即时生效、无需重启。
非法输入（如写错的 CIDR）会阻止保存；已生效的运行时编辑非法值则保留上次
有效值，不会中断正在进行的抓取。无设置服务的环境（如 headless）自动回落
到组合配置。

...
//     192.168.31.0/24). System-resolved addresses inside the allowlist are
...
export const Config = z.object({
  ...
  lanCidr: z.string().default('192.168.31.0/24'),
...
export function apply(ctx, config) {
  ...
  ctx.web.registerFetchProvider(new FakeIpFetchProvider(limits, net));
}
...
export class FakeIpFetchProvider {
  ...
  async requestOnce(url, signal) {
    try {
      const { addresses, path } = await resolveFakeIpAware(url.hostname, signal, this.net);
      return await requestPinned(url, addresses, {
        'user-agent': this.limits.userAgent,
        accept: 'text/html,application/xhtml+xml,text/*;q=0.9,application/json;q=0.8',
      }, signal, path === 'lan' && this.net.lanInsecure === true);
    } catch (error) {
